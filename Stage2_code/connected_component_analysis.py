...
# 3. Produces an MxM connectivity matrix of zeros and ones
to represent how those M events are related (similar = 1, dissimilar=0)
...
def compute(time_window, event1, event2):
    ...
    for i in range(len(event1)):
        ...
        if start_time < time1 < end_time:
            if start_time < time2 < end_time:

...
def generate_histograms(repeaters_file,station_location_file):
	...
	intervals=[]  # time interval between events. 
	mean_mag=[]  # mean magnitude of two events. 
	M=[] # the difference between the two magnitudes. 
	...
	for line in my_file:
		...
		[longitude1, latitude1, depth1, mag1, loc_type] = util_general_functions.get_info_from_sac(file1);
		file2=mydirectory+use_station+"*"+name2;	
		[longitude2, latitude2, depth2, mag2, loc_type] = util_general_functions.get_info_from_sac(file2);

...
def mendocino_main_program(Network_repeaters_list, families_summaries, station_locations, mapping_code,
                           mapping_data_general, mapping_data_specific):
...
def anza_main_program(Network_repeaters_list, families_summaries, station_locations, mapping_code, mapping_data_general,
                      mapping_data_specific):
    pairwise_gmt(Network_repeaters_list, station_locations, mapping_code, mapping_data_general, mapping_data_specific);
...
def pairwise_gmt(repeaters_file, station_location_file, mapping_code, mapping_data_general, mapping_data_specific):
    ...
    call(mapping_code + '/map_view_repeaters.gmt 0 0 0 ' + mapping_data_general + ' ' + mapping_data_specific,
         shell=True)  # this is to allow the script to handle both single-station and multi-station files.
...
def mendocino_familywise_gmt(families_summaries, mapping_code, mapping_data_general, mapping_data_specific):
    ...
    for line in input_file:
        ...
        if slip_rate > -1.0:
            if "hypodd" in fam_loctype:
                ...
                outputfile3.write(
                    str(mean_lon) + " " + str(mean_lat) + " -" + str(mean_depth) + " " + str(slip_rate) + "\n");
            else:
                ...
                outputfile4.write(
                    str(mean_lon) + " " + str(mean_lat) + " -" + str(mean_depth) + " " + str(slip_rate) + "\n");
            output_nums.write(
                str(mean_lon) + " " + str(mean_lat) + " " + str(mean_depth) + " " + str(len(fam_lon)) + " \n");
    ...
    call([mapping_code + '/microseismicity_map.gmt', mapping_data_general, mapping_data_specific],
         shell=False)  # makes the depth profile of red dots for repeaters.
    call([mapping_code + '/zoomed_in_slip_depth.gmt', mapping_data_general, mapping_data_specific],
         shell=False)  # makes the depth profile with colored dots for slip rate
    call([mapping_code + '/zoomed_in_num_depth.gmt', mapping_data_general, mapping_data_specific],
         shell=False)  # makes the depth profile with colored dots for slip rate
    call([mapping_code + '/very_zoomed_in_slip_depth.gmt', mapping_data_general, mapping_data_specific],
         shell=False)  # makes the depth profile with colored dots for slip rate.
    call([mapping_code + '/cross_section_plus_historical.gmt', mapping_data_general, mapping_data_specific],
         shell=False)  # makes the depth profile and adds recent M5 events.
    call([mapping_code + '/very_zoomed_in_plus_historical.gmt', mapping_data_general, mapping_data_specific],
         shell=False)  # makes the zoomed in graph with M5s.
...
def anza_familywise_gmt(families_summaries, mapping_code, mapping_data_general, _mapping_data_specific):
    input_file = open(families_summaries, 'r');
    ...
    call([mapping_code + '/repeaters_profile.gmt', mapping_data_general],
         shell=False)  # makes the depth profile of repeaters.
    return;

...
def configure(station_tuple, stage2_dir, metric, cutoff, statistic, freq_method,
    ...
    MyParams = Params(station_name=station_name, metric=metric, cutoff=cutoff, statistic=statistic,
                      ...
                      magnitude_difference_tolerance=magnitude_difference_tolerance,
                      min_freq_inst=min_freq_inst, max_freq_inst=max_freq_inst,
                      lowest_chosen_frequency=lowest_chosen_frequency,
                      highest_chosen_frequency=highest_chosen_frequency,
...
def print_out_statements(MyParams):
    if MyParams.metric == "coh":
        print("\n-------------------------------------");
        print("DEFINING REPEATING EARTHQUAKES FOR STATION %s" % MyParams.station_name);
        ...
        print("DEFINING REPEATING EARTHQUAKES FOR STATION %s " % MyParams.station_name);
        print("\nYou have chosen to define repeaters as: ");
        print("XCorr cutoff = %f." % MyParams.cutoff);
        if MyParams.plot_arg == 1:
            print("You have chosen to view plots of all of your repeaters.  ");
        print("-------------------------------------\n");
    else:
...
def read_data_input_file(filename):
    ...
    event1name, event2name, ev1mag, ev2mag, ev1dist, ev2dist = [], [], [], [], [], [];
    xcorr, coh_collection = [], [];
    for line in ifile:
        coh = [];
        temp = line.split();
    ...
    ifile.close();
    Candidates_coh = Candidate_data(event1name=event1name, event2name=event2name, ev1mag=ev1mag, ev1dist=ev1dist,
                                    ev2mag=ev2mag, ev2dist=ev2dist, xcorr=xcorr, coh_array=coh_collection);
...
def read_snr_input_file(filename):
    ...
    f_axis1, snr1, f_axis2, snr2, event1name, event2name = [], [], [], [], [], [];
    ...
    ifile.close();
    Candidates_snr = SNR_data(event1name=event1name, event2name=event2name, f_axis1=f_axis1, snr1=snr1, f_axis2=f_axis2,
                              snr2=snr2);
...
def compute(MyParams, Candidates_coh, Candidates_snr):
    coh_all, xcorr_all, name1_all, name2_all = [], [], [], [];
    dist1_all, mag1_all, dist2_all, mag2_all = [], [], [], [];
    min_freq_all, max_freq_all = [], [];
    coh_cre, xcorr_cre, name1_cre, name2_cre = [], [], [], [];
    dist1_cre, mag1_cre, dist2_cre, mag2_cre = [], [], [], [];
    ...
    for i in range(len(Candidates_coh.event1name)):
        ...
        if Candidates_coh.event1name[i] != Candidates_snr.event1name[i]:
            print("We have a problem! " + Candidates_coh.event1name[i] + " and " + Candidates_snr.event1name[
                i] + " do not match. Go check the input files!\n");
            continue;
        if Candidates_coh.event2name[i] != Candidates_snr.event2name[i]:
            print("We have a problem! " + Candidates_coh.event2name[i] + " and " + Candidates_snr.event2name[
                i] + " do not match. Go check the input files!\n");
            continue;
        ...
        if Candidates_coh.ev1dist[i] > 3000 and Candidates_coh.ev2dist[i] > 3000:
            print("We have a problem!  " + Candidates_coh.event1name[i] + " and " + Candidates_coh.event2name[
                i] + " are both located too far away.\n");
            distance = 25;  # THIS IS A TOTAL GUESS.  WE FOUND ONE EVENT WHERE THIS IS NEEDED.

        # Determine maximum frequency we're looking at for a pair of events (Mean magn only, nothing to do with SNR);
        if MyParams.metric == "corr":  # set to default values if using corr, because they don't matter.
            ...
            min_freq, max_freq = determine_freqs_by_SNR(Candidates_snr.f_axis1[i], Candidates_snr.snr1[i],
                                                        Candidates_snr.f_axis2[i], Candidates_snr.snr2[i],
                                                        MyParams.snr_cutoff, MyParams.highest_chosen_frequency,
                                                        MyParams.lowest_chosen_frequency);
        ...
        index_1 = int(np.round(
            len(coh) * (min_freq - MyParams.min_freq_inst) / (MyParams.max_freq_inst - MyParams.min_freq_inst)));
        index_top = int(np.round(
            len(coh) * (max_freq - MyParams.min_freq_inst) / (MyParams.max_freq_inst - MyParams.min_freq_inst)));
        ...
        if (max_freq - min_freq) <= MyParams.Minimum_frequency_width:
            ...
            if abs(Candidates_coh.ev1mag[i] - Candidates_coh.ev2mag[i]) <= MyParams.magnitude_difference_tolerance:
                # if we have similar magnitudes
                if MyParams.metric == "coh":
    ...
    Total_results = Results_summary(name1=name1_all, name2=name2_all, xcorr_value=xcorr_all, coh_value=coh_all,
                                    min_freq=min_freq_all, max_freq=max_freq_all, dist1=dist1_all, dist2=dist2_all,
                                    mag1=mag1_all, mag2=mag2_all);
    CRE_results = Results_summary(name1=name1_cre, name2=name2_cre, xcorr_value=xcorr_cre, coh_value=coh_cre,
                                  min_freq=min_freq_cre, max_freq=max_freq_cre, dist1=dist1_cre, dist2=dist2_cre,
                                  mag1=mag1_cre, mag2=mag2_cre);

    print("\nSTATION " + MyParams.station_name + ":\n");
    print("%d Repeaters Found" % (len(CRE_results.name1)));
    print("Out of %d possible event pairs." % len(Total_results.name1));
    print("%d event pairs had low SNR (below SNR=%f).\n" % (low_signal_counter, MyParams.snr_cutoff));
...
def outputs(MyParams, Total_results, CRE_results, mapping_data_general, mapping_data_specific, mapping_code):
    ...
    if len(CRE_results.name1) >= 0:  # should be len(CRE_results.name1) (during non-debug operation)
        ...
        make_histograms_plots.make_repeaters_map(MyParams, mapping_data_general, mapping_data_specific,
                                                 mapping_code);  # making a gmt plot of repeating event locations.
        ...
        if len(CRE_results.name1) >= 1:
            ...
            if MyParams.metric == "corr":
                make_histograms_plots.make_xcorr_histogram(Total_results.xcorr_value, CRE_results.xcorr_value,
                                                           MyParams.station_name, MyParams.output_dir);
            if MyParams.metric == "coh":
                make_histograms_plots.make_coherence_histogram(Total_results.coh_value, CRE_results.coh_value,
                                                               MyParams.station_name, MyParams.output_dir);
                make_histograms_plots.make_xcorr_histogram(Total_results.xcorr_value, CRE_results.xcorr_value,
                                                           MyParams.station_name, MyParams.output_dir);
                make_histograms_plots.make_scatter_coh_xcorr(Total_results.xcorr_value, Total_results.coh_value,
                                                             CRE_results.xcorr_value, CRE_results.coh_value,
                                                             Total_results.dist1, MyParams.station_name,
                                                             MyParams.output_dir);
...
def write_outfiles(MyParams, Total_results, CRE_results):
    Header_string = "Choices and Parameters: metric=" + str(MyParams.metric) + "; cutoff=" + str(
        MyParams.cutoff) + "; snr_cutoff=" + str(MyParams.snr_cutoff) + "; magnitude_difference=" + str(
        MyParams.magnitude_difference_tolerance) + "; statistic = " + MyParams.statistic + "; Available frequencies = " + str(
        MyParams.lowest_chosen_frequency) + " to " + str(MyParams.highest_chosen_frequency) + ".\n"
    ...
    for i in range(len(Total_results.name1)):
        ...
        if time_ev2 >= time_ev1:  # the case where event2 is later in time.
            ...
            Total_results.min_freq[i], Total_results.max_freq[i], Total_results.mag1[i], Total_results.dist1[i],
            Total_results.mag2[i], Total_results.dist2[i]));
        if time_ev1 > time_ev2:  # the case where event1 is later in time (unusual one)
            ...
            Total_results.min_freq[i], Total_results.max_freq[i], Total_results.mag2[i], Total_results.dist2[i],
            Total_results.mag1[i], Total_results.dist1[i]));
    ofile.close();
    ...
    for i in range(len(CRE_results.name1)):
        ...
        if time_ev2 >= time_ev1:  # the case where event2 is later in time.
            ...
            CRE_results.min_freq[i], CRE_results.max_freq[i], CRE_results.mag1[i], CRE_results.dist1[i],
            CRE_results.mag2[i], CRE_results.dist2[i]));
        if time_ev1 > time_ev2:  # the case where event1 is later in time (unusual one)
            ofile.write("%s %s %f %f %f %f %f %f %f %f \n" % (
            CRE_results.name2[i], CRE_results.name1[i], CRE_results.xcorr_value[i], CRE_results.coh_value[i],
            CRE_results.min_freq[i], CRE_results.max_freq[i], CRE_results.mag2[i], CRE_results.dist2[i],

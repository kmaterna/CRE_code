...
def overall_inputs(families_summaries,station_locations,mapping_data):
	...
	[stations,station_paths]=util_general_functions.get_dirs_for_station(station_locations);
...
def major_plots(myline,stations,station_paths,output_dir, ca_coords, plate_coords,time_window):
	...
	[longitude, latitude, event_time, magnitude, depth, type_of_loc, mean_lon, mean_lat, mean_depth, slip_rate] = util_general_functions.read_family_line(myline);
	...
	[ev1_repeaters, ev2_repeaters, xcorr_repeaters, coherence_repeaters] = read_event_comparison(best_station+'_repeaters_list.txt');
	...
	util_functions_for_viewing_families.make_waveform_plot(family_number, event_names, magnitude, best_station, sac_directory, output_dir);
	...
	coh_array=util_functions_for_viewing_families.make_similarity_matrix(number_of_events, event_names,ev1_non_repeaters,ev2_non_repeaters,coherence_non_repeaters);
	xcorr_array=util_functions_for_viewing_families.make_similarity_matrix(number_of_events, event_names,ev1_non_repeaters,ev2_non_repeaters,xcorr_non_repeaters)

	# Plotting arrays of cross-correlation and coherence
	a1.scatter(xx, yy, c=coh_array, s=round(30000/(number_of_events*number_of_events)), marker='s',cmap='jet', vmin=0.5, vmax=1);

...
Indv_station_repeaters = collections.namedtuple('Indv_station_repeaters', ['station_names', 'station_data']);
...
def compute(Individual_station_reps):
    ...
    for i in range(len(Individual_station_reps.station_data)):
        ...
        for j in range(i + 1, len(Individual_station_reps.station_data)):
            ...
            print("Comparing " + Individual_station_reps.station_names[i] + " with " +
                  Individual_station_reps.station_names[j] + "...");

            for val in myarray1:  # Find event pairs that were detected as repeaters in both stations
                if val in myarray2:
                    winners.append(val);
                    recorded_stations.append(
                        [Individual_station_reps.station_names[i], Individual_station_reps.station_names[j]]);

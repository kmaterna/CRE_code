...
def get_summary_statistics(families_summaries):
    ...
    [avg_cov, n_families, avg_n, max_n, mean_timespan, n_short_families, slip_rate_boxes, unc_boxes] = compute(box_dims,
                                                                                                               time_window,
                                                                                                               make_slip_rate_cutoff,
                                                                                                               MyFamilies);
    write_outputs(output_filename, avg_cov, n_families, avg_n, max_n, mean_timespan, n_short_families,
                  make_slip_rate_cutoff, box_dims, slip_rate_boxes, unc_boxes);
...
def inputs(families_summaries):
    ...
    for line in myfile:
        [sing_lon_array, sing_lat_array, sing_time_array, sing_mag_array, sing_dep_array, _, _,
         _, _, _] = util_general_functions.read_family_line(line);
    ...
    MyFamilies = Family_sum_arrays(lon_array=lon_array, lat_array=lat_array, dep_array=dep_array, mag_array=mag_array,
                                   event_times_array=event_times_array, cov=cov_array, n_events=n_events_array,
                                   event_timespan=event_timespan_array, family_id=id_array);
...
def compute(box_dims, time_window, make_slip_rate_cutoff, MyFamilies):
    ...
    for mybox in box_dims:
        [slip_rate1, unc_1] = get_slip_rate_stats(mybox, make_slip_rate_cutoff, time_window, MyFamilies,
                                                  is_long_enough_array);  # for box 1: the lower region
                    ...
                    if (dep_array[i][j] < max_dep) and (dep_array[i][j] > min_dep):
                        if (event_times_array[i][j] < time_window[1]) and (event_times_array[i][j] > time_window[0]):
                            if is_long_enough_array[i] == 1:
                                ...
                                family_code_number.append(
                                    id_array[i])  # an array like [0 0 0 0 1 1 2 2 ...] for coding each event by family.
    number_of_families = len(set(family_code_number));  # the unique number of families that we observed in the blob.

    slip_rate = get_slip_rate(event_timing, event_magnitude, time_window,
                              number_of_families);  # slip rate from the full dataset
...
def write_outputs(filename, avg_cov, n_families, avg_n, max_n, mean_timespan, n_short_families, slip_rate_cutoff,
                  box_dims, slip_rate_boxes, unc_boxes):

...
def master_waveform_viewer_snr(station_name, event1, event2, f1=1.0, f2=15.0, snr_cutoff=5.0, raw_sac_dir="",
                               output_dir=""):
    """
    ...
    print("For station %s " % station_name)
    ...
    event1_base = event1.split('/')[-1];  # e.g., B045.PB.EHZ..D.2012.127.091831.71776880.sac, without directory
    event2_base = event2.split('/')[-1];  # e.g., B045.PB.EHZ..D.2012.127.091831.71776880.sac, without  directory
    ...
    subprocess.call(
        "gcc -o coherence_setup " + path_to_code + "/coherence_setup_raw_and_filtered.c " + compiler_arguments,
        shell=True)
    ...
    subprocess.call(
        "./coherence -i coh_input_temp.txt -f " + str(frequency) + " -n " + str(pps_c) + " > coh_output.txt",
        shell=True)
    ...
    make_coh_snr_plot.make_coh_snr_plot(output_dir, event1_base, event2_base, "two_filtered_waveforms.txt",
                                        "coh_output.txt", frequency, min_freq, max_freq, snr_cutoff, plot_overlay,
                                        pretty_plot);

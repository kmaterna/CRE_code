...
def make_inter_event_time_histogram(station_name,filename,output_dir):
	...
	for line in my_file:
		...
		name1=ev1[-28:-13]
		name2=ev2[-28:-13]
		event_time1=float(name1[0:4])+float(name1[5:8])/365.24+float(name1[9:11])/(24*365.24)+float(name1[11:13])/(24*60*365.24)+float(name1[13:15])/(24*60*60*365.24)
...
def make_repeater_seismograms(MyParams):
	...
	for line in my_file:
		...
		master_waveform_viewer_snr.master_waveform_viewer_snr(MyParams.station_name, event1,event2,f1,f2,MyParams.snr_cutoff,MyParams.raw_sac_dir,MyParams.output_dir);
	my_file.close();
	return;
